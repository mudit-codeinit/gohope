	...
	public function push_notification(Request $request){ 
		  //API URL of FCM
		   $message = isset($request->message) && !empty($request->message) ? $request->message :'this is a test';
			...
			if($request->notification ==  'reporter'){
					$result =  push_repoter_notification_android( $request->device_token ,$message ); 
			}
			if($request->notification ==  'driver'){
				$result = 	push_driver_notification_android( $request->device_token ,$message ); 
			}
			return $result;
	}
